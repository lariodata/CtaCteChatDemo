/* =====================================================================
   chat_deuda_por_zona
   Devuelve deuda total de UNA zona puntual (reporte para GERENTES).

   Version simple (demo): @nro_zona es OBLIGATORIO, no se admite "todas
   las zonas" (recorrer la tabla completa de clientes via el obrero legacy
   no escala con millones de filas). El gerente pide una zona por vez,
   igual que chat_clientes_por_zona.

   Mismo patron que chat_clientes_por_zona: una sola llamada a
   ctacte_armado_sql con @nrocli = 0 (= toda la zona pedida).

   VALIDACION:
     EXEC dbo.chat_deuda_por_zona @nro_zona = 1;
   ===================================================================== */
USE [DATOS_COBRANZAS];
GO
SET ANSI_NULLS ON;
...
GO
IF OBJECT_ID('dbo.chat_deuda_por_zona','P') IS NOT NULL
    DROP PROCEDURE dbo.chat_deuda_por_zona;
GO
CREATE PROCEDURE dbo.chat_deuda_por_zona
...
BEGIN
    ...
    /* Rollup por cliente primero (misma formula que el resto de los chat_*),
       despues totaliza la zona. */
    ;WITH por_cliente AS (
        SELECT
            c.nrocli,
    ...
    SELECT
        ...
        CASE WHEN COUNT(*) = 0 THEN 0 ELSE SUM(deuda_total) / COUNT(*) END AS deuda_promedio
    FROM por_cliente;
